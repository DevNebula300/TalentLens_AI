...
The entire application stack (Frontend, Backend, and Database) is containerized and ready for production deployment using Docker Compose.
...
This master `docker-compose.yml` will:

- Spin up the `pgvector` database on an internal secure network.
- Build and run the FastAPI backend on port `8000`.
- Build the Next.js frontend (using standalone output mode for maximum optimization) and serve it on port `3000`.

_Note: In a production environment, you should configure `ALLOWED_ORIGINS` for the backend and `NEXT_PUBLIC_API_URL` for the frontend build arguments to match your domain._
